...
for (xc in 1:n){ #let each dot be the center dot, one at a time
  xdists <- X - X[xc]  #compute distance from all X-values to center dot ax-value
  ...
  curve(lmc$coef[1] + lmc$coef[2]*x + I(lmc$coef[3]*x^2), #draw line on neighborhood using from and to
  ...
  lines(lfit[1:xc] ~ X[1:xc] + X[1:xc]^2, col="gray")
