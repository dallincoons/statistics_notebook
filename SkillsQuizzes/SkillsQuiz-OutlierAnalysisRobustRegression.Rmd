...
my_surface <- acast(my_surface, X2 ~ X1, value.var = "Z")
...
predict(mylm, data.frame(X1 = 26.03622, X2 = 1.409711), type="response")
...
Point 4, the outlier in the Y direction is very obvious in the diagnostic plots and cooks distance. Point 5, the outlier in the x direction blends in very well with the other points and it's impossible to detect.
...
```{r}
util <- Utilities %>% dplyr::select(c(gasbill, year, temp))
pairs(util)
```
...
```{r}
ulm <- lm(gasbill ~ year + temp + I(temp^2), data=util)
rulm <- rlm(gasbill ~ year + temp + I(temp^2), data=util)

summary(ulm)
summary(rulm)

par(mfrow=c(1,3))
plot(ulm, which=c(1,4,5), col=rgb(rulm$w,1,.5))
```
...
```{r}
pairs(theData2, col = ifelse(rownames(theData2)==3, "red", "blue"))
...
pairs(cbind(R = mylm2$residuals, Fit = mylm2$fit, theData2))
...
```{r}
rlm2 <- rlm(Y ~ X1 + X2 + X3, data = theData2)
summary(rlm2)

olm <- lm(Y ~ X1 + X2 + X3, data = theData2)
summary(olm)

olm <- lm(Y ~ X1 + X2 + X3, data = theData2[-3,])
summary(olm)
```
