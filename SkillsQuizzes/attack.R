...
test.lm <- lm(Y ~ X1 + X2 + X3 + X4 + X6 + X9, data = dat)
...
pairs(cbind(R=test.lm$res, fit=test.lm$fit, dat), pch=16, cex=1, panel=panel.smooth, col.smooth="skyblue4", col=factor(dat$X5))
