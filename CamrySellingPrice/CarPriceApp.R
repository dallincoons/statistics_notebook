...
server <- function(input, output) {
  ...
  output$scatterPlot <- renderPlot({
    ...
    car.lm.log <- lm(log(Price) ~ Mileage, data=cardata)
  ...
  output$message1 <- renderText({
    ...
    paste('The predicted value of the car with ', 
          milesSelling, 
          paste(' miles is $', trunc(predictedSelling*10^2)/10^2),
          ' which means you drove the vehicle for ',
          milesSelling - milesPurchase,
          paste(' miles with a net operating', ifelse((pricePurchase - predictedSelling) > 0, 'loss', 'gain'),' of', trunc(abs(pricePurchase - predictedSelling)*10^2)/10^2),
          sep='')
    ###----------- END EDIT AREA --------------------###  
  })
