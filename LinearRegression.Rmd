...
<div class="hoverchunk"><span class="tooltipr">
ggplot(
  <span class="tooltiprtext">This is used to declare a basic ggplot graphic area. It is essentially a blank graphic object until aesthetics and geometries are added to it.</span>
</span><span class="tooltipr">
YourDataSet,
  <span class="tooltiprtext">This is the name of your data set that you will be using to create the graphic. If multiple different data sets are to be used, then it is better to place the data set name inside of each geom_... instead of in the main ggplot(...) statement.</span>
</span><span class="tooltipr">
aes(
  <span class="tooltiprtext">This controls the aesthetics (or "axes") of the graph to be certain values for all geometries and added elements.</span>
</span><span class="tooltipr">
x=XcolumnName,
  <span class="tooltiprtext">x= is required to specify the x-axis of a graphic. The name of a column that will become the variable of the x-axis should be placed after the x= statement.</span>
</span><span class="tooltipr">
y=YcolumnName
  <span class="tooltiprtext">y= is required to specify the y-axis variable of the graphic.</span>
</span><span class="tooltipr">
)
...
+
  <span class="tooltiprtext">The plus sign indicates that further elements will be added to the graphic.</span>
</span><br/><span class="tooltipr">
&nbsp; geom_point(
  <span class="tooltiprtext">The geom_point(...) function is used to add points as a geometric object to the plot. It puts "dots on the plot".</span>
</span><span class="tooltipr">
&nbsp;
  <span class="tooltiprtext">Many optional arguments can be placed inside of geom_point(...) including aes(...) statements, color= statements, size= statements, pch= statements, and so on.</span>
</span><span class="tooltipr">
)
...
&nbsp; geom_smooth(
  <span class="tooltiprtext">This geometry puts a smoothin line onto the data. The default is to place a loess curve as the smooth line.</span>
</span><span class="tooltipr">
method=
  <span class="tooltiprtext">There are many ways to smooth a line, the two most popular being to use "loess" or "lm". To get the least squares regression line, use "lm".</span>
</span><span class="tooltipr">
"lm",
  <span class="tooltiprtext">This puts the least squares regression line onto the plot. You could also use "loess" to get the loess curve instead.</span>
</span><span class="tooltipr">
&nbsp;se=
  <span class="tooltiprtext">Should the confidence band for the regression line using the "standard error" of the slope and intercept be drawn on the plot?</span>
</span><span class="tooltipr">
FALSE
  <span class="tooltiprtext">Unless you know how to interpret this confidence band, it is wise to leave it off and just display the least squares regression line.</span>
</span><span class="tooltipr">
)
...
<a href="javascript:showhide('ggplotCustom')">
<div class="hoverchunk"><span class="tooltipr">
ggplot(
  <span class="tooltiprtext">This is used to declare a basic ggplot graphic area. It is essentially a blank graphic object until aesthetics and geometries are added to it.</span>
</span><span class="tooltipr">
...
&nbsp; geom_point(
  <span class="tooltiprtext">This geometry adds points to a plot.</span>
</span><span class="tooltipr">
color="SomeColor",
  <span class="tooltiprtext">Change the color of all dots. If you want to create colors according to a column of data, use aes(color=columnName) instead.</span>
</span><span class="tooltipr">
&nbsp;size=2,
  <span class="tooltiprtext">Changes the size of all dots. If you want to have size depend on a column use aes(size=columnName) instead.</span>
</span><span class="tooltipr">
&nbsp;shape=1
  <span class="tooltiprtext">Controls the shape of the dot. Options include an integer of 0, 1, ... 25, (which is equivalent to pch values), a single character (which uses that character as the plotting symbol), a "."" to draw the smallest rectangle that is visible (i.e., about one pixel), an NA to draw nothing.</span>
</span><span class="tooltipr">
)
  <span class="tooltiprtext">Closing parenthesis for the geom_point(...) function.</span>
</span><span class="tooltipr">
...
linetype=1,
  <span class="tooltiprtext">linetype allows the options of 0 = blank, 1 = solid, 2 = dashed, 3 = dotted, 4 = dotdash, 5 = longdash, 6 = twodash, which can be specified by number or name, like "dashed" for example.</span>
</span><span class="tooltipr">
&nbsp;color="SomeColor"
...
<div id="ggplotCustom" style="display:none;">
```{r, echo=FALSE}
ggplot(cars, aes(speed, dist)) +
  geom_point(aes(speed, dist), color = "darkorchid", size = 2, shape = 1) +
  geom_smooth(method = lm, se = FALSE, linetype = 1, color = "skyblue") +
  theme_bw()
```
</div>
...
| $b_0$ | "b-zero" | Estimated y-intercept | $b_0 = \bar{Y} - b_1\bar{X}$ | `b_0 <- mean(Y) - b_1*mean(X)` Or `b_0 <- coef(mylm)[1]`| 
| $b_1$ | "b-one" | Estimated slope | $b_1 = \frac{\sum X_i(Y_i - \bar{Y})}{\sum(X_i - \bar{X})^2}$ | `b_1 <- sum( X*(Y - mean(Y)) ) / sum( (X - mean(X))^2 )` Or `b_1 <- coef(mylm)[2]`| 
...
| $MSE$ | "mean squared error" | Estimate of $\sigma^2$ | $MSE = \frac{SSE}{n-p}$ | `sum( lmObject$res^2 ) / (n - p)` Or `summary(mylm)$sigma^2` |
| $\sqrt{MSE}$ | "root mean squared error" | Estimate of $\sigma$, called "residual standard error" by R. | $\sqrt{MSE} = \sqrt{\frac{SSE}{n-p}}$ | `sqrt(sum( lmObject$res^2 ) / (n - p))` Or `summary(mylm)$sigma` |
...
| $R^2$ | "R-squared" | Proportion of variation in Y explained by the regression | $R^2 = \frac{SSR}{SSTO} = 1 - \frac{SSE}{SSTO}$ | `SSR/SSTO` Or `summary(mylm)$r.squared`|
| Adjusted $R^2$ | "Adjusted R-squared" | Proportion of variation in Y explained by the regression with penalty included for too many model parameters | $R^2_\text{adj} = 1 - \frac{(n-1)}{(n-p)}\frac{SSE}{SSTO}$ | `1 - (n-1)/(n-p)*SSE/SSTO` Or `summary(mylm)$adj.r.squared`|
...
Most inference in regression is focused on the slope, $\beta_1$. Recall that the interpretation of $\beta_1$ is the amount of increase (or decrease) in the expected value (average value) of $Y$ per unit change in $X$. 

Two types of inference about $\beta_1$ are of interest.
...
ggplot(mtcars, aes(hp, mpg)) +
  geom_point(aes(hp, mpg, color = factor(am))) +
  geom_smooth(method = lm, aes(color = factor(am)), se = FALSE, fullrange = TRUE)
```
</div>
...
```{r, echo=FALSE}
ggplot(mtcars, aes(hp, mpg)) +
  geom_point(aes(hp, mpg, color = factor(am)), size = 2, shape = 1) +
  geom_smooth(method = lm, se = FALSE, linetype = 1, aes(color = factor(am))) +
  theme_bw() +
  labs(color = "am")
```
</div>
